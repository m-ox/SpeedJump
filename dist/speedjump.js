...
function findScormVideo() {
    ...
    return (courseDocument?.querySelector('#leanback-video-id0') ||
...
function findVideoElement(rootDocument = document) {
    ...
    if (scormVideo instanceof HTMLVideoElement)
        return scormVideo;
...
function updateSpeed() {
    const currentRate = videoTag?.ownerDocument?.getElementById('current-rate') ||
...
function createRoot() {
    if (document.getElementById('sj-menu-root'))
        return;
...
function handleConfirmElementId() {
    const input = document.getElementById('sj-video-element-input');
...
function populateMenu() {
    ...
    if (!menuRoot || document.getElementById('sj-video-element-input'))
        return;
